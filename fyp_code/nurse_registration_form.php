...
function generateNurseID($connect) {
    $query = "SELECT nurseid FROM nurse ORDER BY nurseid DESC LIMIT 1";
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        $numericPart++;
        return '7' . str_pad($numericPart, 4, '0', STR_PAD_LEFT); // Format new ID
    }
    return '70001'; // First ID if no records exist
}
...
function sendOTP($email, $otp) {
    $subject = "Your OTP for Nurse Registration";
...
if (isset($_POST['nurse_register'])) {
    if (
        ...
        $nurseid = generateNurseID($connect); // Generate new Nurse ID
        ...
        $check_query = "SELECT nurseid, email, phonenum, username FROM nurse WHERE email = ? OR phonenum = ? OR username = ?";
        if ($check_stmt = mysqli_prepare($connect, $check_query)) {
            ...
            if (mysqli_stmt_num_rows($check_stmt) > 0) {
                mysqli_stmt_bind_result($check_stmt, $existingNurseID, $existingEmail, $existingPhoneNum, $existingUsername);
                while (mysqli_stmt_fetch($check_stmt)) {
                    ...
                    } elseif ($existingNurseID === $nurseid) {
                        echo "<script>alert('Nurse ID already exists. Please use a different Nurse ID.');</script>";
                    }
                }
            } else {
                $otp = generateOTP();
                ...
                $_SESSION['nurse_registration_data'] = [
                    'nurseid' => $nurseid,
                    'firstname' => $firstname,
                    'lastname' => $lastname,
                    'username' => $username,
                ...
                if (sendOTP($email, $otp)) {
                    echo "<script>
                        ...
                        window.location.href = 'nurseOTPVerification.php';
                    </script>";
                    exit;
                } else {
...
<div class="container">
    <div class="register-form">
        <h1 style="text-align: center;">Nurse Registration Form</h1>
        <p style="color: #c30010; text-align: center; font-size: 12px;"><i>*Required Field</i></p>
        <form action="nurse_registration.php" method="POST" id="nurseRegistrationForm" onsubmit="showLoadingOverlay()">
            
            <label>*First Name:</label><br>
            <input type="text" name="firstname" placeholder="First Name" id="nurseFirstName" required><br><br>

            <label>*Last Name:</label><br>
            <input type="text" name="lastname" placeholder="Last Name" id="nurseLastName" required><br><br>

            <label>*Username:</label><br>
            <input type="text" name="username" placeholder="Username" id="nurseUsername" required><br>
            <div id="nurseUsernameMessage"></div><br>

            <label>*Password:</label><br>
            <input type="password" name="password" placeholder="Password" id="nursePassword" 
            ...
            <input type="text" name="address" placeholder="Address" id="nurseAddress" required><br>
...
<div id="nursePhoneNumMessage"></div><br>
            ...
            <input type="email" name="email" placeholder="email@mail.com" id="nurseEmail" required><br>
            <div id="nurseEmailMessage"></div><br>
            ...
            <label for="nurseRole">*Responsible symptoms: </label>
            <div>
                <input 
                    type="text" 
                    name="role" 
            ...
            <br>
            <button type="submit" class="register-button" name="nurse_register">Register</button>
            <p class="login"> &#128008;Already Sign Up? <a href="nurse_login.php" onclick="showLoadingOverlay()">Login in here.</a></p>
        </form>
    </div>
</div>
