...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $query = "SELECT nurseid FROM nurse WHERE email = ?";
    ...
    if ($stmt) {
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            $tokenQuery = "UPDATE nurse SET reset_token = ? WHERE nurseid = ?";
            $tokenStmt = mysqli_prepare($connect, $tokenQuery);

            if ($tokenStmt) {
                mysqli_stmt_bind_param($tokenStmt, "si", $token, $nurseid);
                mysqli_stmt_execute($tokenStmt);

                // Send the reset password link via email
                $resetLink = "http://localhost/Vet_System_Fyp/fyp_code/nurseResetPassword.php?token=" . $token;
                ...
                if (mail($email, $subject, $message, $headers)) {
                    echo "<script>alert('Password reset link has been sent to your email.'); window.location.href = 'nurse_login.php';</script>";
                } else {
                    echo "<script>alert('Error sending email. Please try again later.');</script>";
                }
            }
        } else {
            echo "<script>alert('Email not found. Please check and try again.'); window.location.href = 'nurseForgotPassword.php';</script>";
        }
...
<head>
    ...
    <title>Nurse Forgot Password | VCMS</title>
...
<div class="cute-animal"></div>
    ...
    <form action="nurseForgotPassword.php" method="POST" onsubmit="showLoadingOverlay()">
        <label for="nurse_email">Enter your registered email:</label>
        <input type="email" name="nurse_email" id="nurse_email" required>
