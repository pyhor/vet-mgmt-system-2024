...
function generateAppID($connect) {
    $query = "SELECT appid FROM appointment_temp ORDER BY appid DESC LIMIT 1";
...
// Fetch symptom, pettype, and vet details based on schedule ID
...
$appName = $scheduleDetails['appname']; // Fetch appname

// If appname is not set, provide a default value
if (!$appName) {
    $appName = "Default Appointment Name"; // Replace with an appropriate default
}
...
$insertQuery = "
    INSERT INTO appointment_temp (appid, petownerid, symptom, appstatus, vetid, date, time, petname, pettype, department, relatedtreatment, relatedpetcare, relatedsymptoms, appname)
    VALUES (?, ?, ?, 'Pending', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
";
...
if ($stmt = mysqli_prepare($connect, $insertQuery)) {
    ...
    if (mysqli_stmt_execute($stmt)) {
        // Update the schedule table to set scheduledstatus to 'Available'
        ...
        echo "<script>alert('Temporary Appointment [Appointment ID: $appId] successfully booked! Click the confirm appointment if you confirm the appointment.'); window.location.href = 'appointment.php';</script>";
    } else {
