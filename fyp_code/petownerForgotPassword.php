...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $email = $_POST['petowner_email'];

    // Check if the email exists in the database
    $query = "SELECT petownerid FROM petowner WHERE email = ?";
    ...
    if ($stmt) {
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            $tokenQuery = "UPDATE petowner SET reset_token = ? WHERE petownerid = ?";
            $tokenStmt = mysqli_prepare($connect, $tokenQuery);

            if ($tokenStmt) {
                mysqli_stmt_bind_param($tokenStmt, "si", $token, $petownerid);
                mysqli_stmt_execute($tokenStmt);

                // Send the reset password link via email
                $resetLink = "http://localhost/Vet_System_Fyp/fyp_code/petownerResetPassword.php?token=" . $token;
                ...
                if (mail($email, $subject, $message, $headers)) {
                    echo "<script>alert('Password reset link has been sent to your email.'); window.location.href = 'petowner_login.php';</script>";
                } else {
                    echo "<script>alert('Error sending email. Please try again later.');</script>";
                }
            }
        } else {
            echo "<script>alert('Email not found. Please check and try again.'); window.location.href = 'petownerForgotPassword.php';</script>";
        }
...
<div class="cute-animal"></div>
    ...
    <form action="petownerForgotPassword.php" method="POST" onsubmit="showLoadingOverlay()">
        <label for="petowner_email">Enter your registered email:</label>
        <input type="email" name="petowner_email" id="petowner_email" required>
