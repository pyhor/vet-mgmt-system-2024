...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Validate the token in the petowner table
    $query = "SELECT petownerid FROM petowner WHERE reset_token = ?";
    ...
    if ($stmt) {
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            $updateQuery = "UPDATE petowner SET password = ?, reset_token = NULL WHERE petownerid = ?";
            $updateStmt = mysqli_prepare($connect, $updateQuery);
            if ($updateStmt) {
                mysqli_stmt_bind_param($updateStmt, "si", $new_password, $petownerid);
                mysqli_stmt_execute($updateStmt);
                echo "<script>alert('Password reset successful. Please login.'); window.location.href = 'petowner_login.php';</script>";
            } else {
                echo "<script>alert('Failed to reset password. Please try again later.'); window.location.href = 'petownerForgotPassword.php';</script>";
            }
        } else {
            echo "<script>alert('Invalid token. Please request a new password reset.'); window.location.href = 'petownerForgotPassword.php';</script>";
        }
...
<html>
<head>
    <title>Pet Onwer Reset Password |VCMS</title>
    <style>
        body {
...
<div class="cute-animal"></div>
    ...
    <form action="petowner_reset_password.php" method="POST" onsubmit="showLoadingOverlay()">
...
<?php
    ...
    echo "<script>alert('Invalid access.'); window.location.href = 'petownerForgotPassword.php';</script>";
}
?>
